...
void Init() {
    ...
    if (hp) { s_GodMode.address = hp; s_GodMode.patched = { 0x90, 0x90, 0x90, 0x90 }; ++s_ScanFound; }

    uintptr_t hpAlt = Pattern::Scan("F3 41 0F 11 16 F3 44 0F 5C E6", base, size);
    if (hpAlt) { s_GodModeAlt.address = hpAlt; s_GodModeAlt.patched = { 0x90, 0x90, 0x90, 0x90, 0x90 }; ++s_ScanFound; }

    uintptr_t ammo = Pattern::Scan("66 44 89 1C 48 49 8B 4A 58", base, size);
    if (ammo) { s_InfAmmo.address = ammo; s_InfAmmo.patched = { 0x90, 0x90, 0x90, 0x90, 0x90 }; ++s_ScanFound; }

    uintptr_t rel = Pattern::Scan("83 B9 ?? ?? ?? ?? 00 0F 8E ?? ?? ?? ?? 48 8B", base, size);
    if (rel) { s_NoReload.address = rel + 7; s_NoReload.patched = { 0x90, 0x90, 0x90, 0x90, 0x90, 0x90 }; ++s_ScanFound; }

    uintptr_t sup = Pattern::Scan("F3 0F 5C ?? ?? ?? ?? ?? F3 0F 11 ?? ?? ?? ?? ?? 0F 2F", base, size);
    if (sup) { s_InfSuppressor.address = sup; s_InfSuppressor.patched.assign(16, 0x90); ++s_ScanFound; }

    uintptr_t rec = Pattern::Scan("F3 0F 58 ?? ?? ?? ?? ?? F3 0F 11 ?? ?? ?? ?? ?? E8 ?? ?? ?? ?? 48 8B", base, size);
    if (rec) { s_NoRecoil.address = rec; s_NoRecoil.patched.assign(16, 0x90); ++s_ScanFound; }

    uintptr_t ohk = Pattern::Scan("F3 0F 11 ?? ?? ?? ?? ?? 80 ?? ?? ?? ?? ?? 00 74", base, size);
    if (ohk) { s_OneHitKill.address = ohk; s_OneHitKill.patched = { 0x0F, 0x57, 0xC0, 0xF3, 0x0F, 0x11, 0x41, 0x00 }; ++s_ScanFound; }

    uintptr_t rfx = Pattern::Scan("F3 0F 5C ?? ?? ?? ?? ?? F3 0F 11 ?? ?? ?? ?? ?? 0F 2F ?? ?? 76", base, size);
    if (rfx) { s_InfReflex.address = rfx; s_InfReflex.patched.assign(16, 0x90); ++s_ScanFound; }

    uintptr_t fall = Pattern::Scan("F3 0F 11 ?? ?? ?? ?? ?? EB ?? F3 0F 10 ?? ?? ?? ?? ?? 0F 2F C1 76 ?? F3 0F 5C", base, size);
    if (fall) { s_NoFallDamage.address = fall; s_NoFallDamage.patched.assign(8, 0x90); ++s_ScanFound; }

    uintptr_t fire = Pattern::Scan("0F 2F ?? ?? ?? ?? ?? 73 ?? F3 0F 58", base, size);
    if (fire) { s_RapidFire.address = fire; s_RapidFire.patched = { 0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0xEB }; ++s_ScanFound; }

    uintptr_t det = Pattern::Scan("F3 0F 11 ?? ?? ?? ?? ?? 48 8B ?? ?? E8 ?? ?? ?? ?? 84 C0 74", base, size);
    if (det) { s_DetectionAddr = det; ++s_ScanFound; }
}
